...
from shap_e.diffusion.gaussian_diffusion import (
    diffusion_from_config as diffusion_from_config_shape,
)
...
class GaussianDreamer(BaseLift3DSystem):
    ...
    def shape(self):
        ...
        model.load_state_dict(
            torch.load("./load/shapE_finetuned_with_330kdata.pth", map_location=device)[
                "model_state_dict"
            ]
        )
        ...
        size = (
            256  # this is the size of the renders; higher values take longer to render.
        )

        cameras = create_pan_cameras(size, device)

        self.shapeimages = decode_latent_images(
            xm, latents[0], cameras, rendering_mode=render_mode
        )
        ...
        rgb = np.concatenate(
            [
                pc.vertex_channels["R"][:, None],
                pc.vertex_channels["G"][:, None],
                pc.vertex_channels["B"][:, None],
            ],
            axis=1,
        )
    ...
    def on_before_optimizer_step(self, optimizer):
        with torch.no_grad():
            if self.true_global_step < 900:  # 15000
                ...
                if (
                    self.true_global_step > 300 and self.true_global_step % 100 == 0
                ):  # 500 100
                    size_threshold = 20 if self.true_global_step > 500 else None  # 3000
                    self.gaussian.densify_and_prune(
                        0.0002, 0.05, self.cameras_extent, size_threshold
                    )
    ...
    def on_test_epoch_end(self):
        ...
        load_ply(
            save_path, self.get_save_path(f"it{self.true_global_step}-test-color.ply")
        )
